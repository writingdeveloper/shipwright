/**
 * Vitest setup for `@repo/observability`.
 *
 ...
 * validation. The Sentry vars (`SENTRY_DSN` / `NEXT_PUBLIC_SENTRY_DSN`) are
 * OPTIONAL and read straight from `process.env` under skip, so setting/deleting
 * them per test deterministically drives the enabled / no-op branches.
 */
