...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      ...
      {
        // The service worker MUST always be revalidated so an update ships on
        // the next visit — a cached `sw.js` would pin users to an old worker.
        // `Service-Worker-Allowed: /` lets the SW claim the whole origin even
        // though it is served from `/sw.js`.
        source: "/sw.js",
        headers: [
          {
            key: "Cache-Control",
